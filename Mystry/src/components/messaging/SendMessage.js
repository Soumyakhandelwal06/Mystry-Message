...
// //
// src/components/messaging/SendMessage.js
import React, { useState, useEffect } from "react";
import { Send, MessageSquare, User, ArrowLeft } from "lucide-react";
import { api } from "../../services/api";
...
export const SendMessage = () => {
  ...
  // Fetch AI suggestions (static on mount)
  useEffect(() => {
    const fetchSuggestions = async () => {
      try {
        const res = await fetch(
        ...
        const data = await res.json();
        if (data.success) setAiSuggestions(data.suggestions.slice(0, 3));
      } catch (err) {
        console.error("AI suggestion fetch error:", err);
      }
    };
  ...
  return (
    <Layout navigate={navigate}>
      <div className="max-w-2xl mx-auto mt-10 p-6 bg-white rounded-2xl shadow-lg border border-gray-100 relative">
        {/* Back Button */}
        <div className="absolute top-4 left-4">
          <Button
            variant="ghost"
